/**
 ** app/api/test/twelio3/route.ts
 * mencoba twelio quick button, 3 button
 */
...
export async function POST(req: Request) {
  try {
    ...
    if (!no_wa || !obat || !apotek_nama) {
      log.debug("test-twilio3", "Parameter no_wa dan email wajib diisi", null);
    ...
    //conten template id
    const ContentSID = "HXfcc3beb71bab5f322274524c5977e97e";
    //variabel
    const variables = JSON.stringify({
      ...
      "8": "konten3", // tekt pada button
      "9": `{test:${no_wa}}`, // id button 1
    });
    ...
    log.info("test-twilio3", "Pesan uji coba berhasil dikirim", {
      sid: message.sid,
      to: no_wa,
      Variable: variables,
    });
    log.debug("test-twilio3", "test data", message);
    ...
    log.error("test-twilio3", "Gagal mengirim pesan Twilio", error, null);
    return NextResponse.json(
